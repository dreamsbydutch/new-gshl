...
keeps roster, cap, matchup, draft, and market status in one compact snapshot;
the complete roster, three-season cap window, contract decisions, five-game
form, pick inventory, offers, listings, and trade leads remain available in an
expandable detail section. The four primary actions open the trade market,
listing controls, UFA offers, and the next matchup. New trade-listing activity
is tracked as read per owner in that browser; the application does not yet store
direct trade proposals or cross-device read receipts.
