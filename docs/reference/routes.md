...
| `/draft-roster-board`           | Public, `noindex`        | `DraftRosterBoard`                            | Dimmed, header-free conference roster overview with a center switch between the complete active-and-inactive owner ladder and an enlarged live clock/recent/upcoming pick view with directional connectors. Compact roster typography leaves room for remaining round-overall pick labels, which fill down columns and disappear on selection. Abbreviated names and fitted text keep each team in its panel. |
| `/draft-roster-board/available` | Public, `noindex`        | `DraftAvailableTvBoard`                       | Shell-free companion TV with as many ranked skaters as the display height supports, the top 10 goalies, and a 14-team roster-makeup table showing each logo and abbreviation, total players, F/D/G roster groups, and C/LW/RW/D/G eligibility.                                                                                                                                                                |
| `/draft-roster-board/live`      | Public, `noindex`        | `DraftLiveTvBoard`                            | Read-only draft flow: active team and clock across the top, height-fitted recent picks descending on the left, current-owner roster in the center, and height-fitted upcoming picks ascending on the right. Small color-coded chevrons connect each pick in its direction of travel. Before the draft, shows the first team and start countdown.                                                              |
...
- `/draft-roster-board` and its companion TV routes are public and bypass the shell entirely. Live TV uses the existing public season, team, player, and draft-pick reads with the shared draft-clock resolver; submitting or undoing picks remains restricted to the authenticated Draft Hub. Its roster follows the pick's franchise when the overview selects a newer season's team record.
