...
- `/draft-roster-board` and its companion TV routes are public, bypass the shell entirely, and are intentionally absent from in-app navigation. Each display is opened only by entering its direct URL and contains no links to the main app or the other TV displays. Live TV uses the existing public season, team, player, and draft-pick reads with the shared draft-clock resolver; submitting or undoing picks remains restricted to the authenticated Draft Hub. Its roster follows the pick's franchise when the overview selects a newer season's team record.
