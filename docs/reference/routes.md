...
| `/lockerroom`              | Active user              | `LockerRoomContent` in `LockerRoomLayout`     | URL-selected team's roster, cap, matchups, trophies, records, and independently seasoned draft-pick history.                                        |
...
- Schedule, Standings, Locker Room, League Office, and Draft layouts place one sticky context-navigation surface before route content. Season selection remains in the global shell instead of being repeated in these page controls. The Locker Room Draft Picks view is the exception: its local selector queries one franchise season at a time without changing the global context. These controls do not add fixed bottom bars or route-specific navigation padding.
- Navigation context is persisted in `gshl-nav-state` and mirrored to route-specific `view`, `season`, `week`, and `owner` query parameters. The global season presets other season-aware surfaces as users move through the application. Valid URL state wins over hydrated persistence; user changes support Back/Forward, while automatic repair replaces the current entry.
