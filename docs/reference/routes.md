...
| `/leagueoffice`            | Active user              | `LeagueOfficeContent` in `LeagueOfficeLayout` | URL-addressable league reference views and role-gated commissioner panels; invalid or fresh state opens Draft Classes. |
...
- Draft and UFA player decisions render as identity-first cards below `lg`, with staged confirmation and 44px controls. Matchup uses vertical category comparisons and expandable performance cards below `lg`. Comprehensive comparison tables use the shared scroll viewport at larger widths.
- Salary Cap remains a cross-season table at all widths with a compact mobile cap summary and nonsticky phone identity columns. Franchise contract history uses cards below `lg` and its comprehensive table above that breakpoint.
