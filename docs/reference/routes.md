...
`/notifications` is an active-user route rendered by `NotificationCenter`, with
a private inbox, eight notification preferences, and browser device management.
The signed-in account controls link to it through an unread-count bell.
...
| `/leagueoffice`                 | Active user              | `LeagueOfficeContent` in `LeagueOfficeLayout` | URL-addressable draft classes and shared league reference views; invalid or fresh state opens Draft Classes. Legacy commissioner-panel links redirect commissioners into `/admin`.                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `/admin`                        | Commissioner             | `AdminContent` in `AdminLayout`               | Dedicated commissioner space for contract administration, users, jobs, Newsroom, image uploads, and direct links to the three public TV displays. Non-commissioners are redirected to League Office.                                                                                                                                                                                                                                                                                                                                                                       |
| `/draft`                        | Active user              | `DraftHubBoard` in `DraftHubLayout`           | Transactional live draft board with compact comparison tables at every width, frozen player names and row actions, sortable stats, selection confirmation, and directional chevrons linking recent and upcoming picks.                                                                                                                                                                                                                                                                                                                                                     |
| `/draft/my-team`                | Active user              | `DraftHubTeamPage`                            | Draft-season roster, picks, then salary cap on mobile; picks first with roster and cap side by side on desktop, for the session's linked owner.                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `/draft-roster-board`           | Public, `noindex`        | `DraftRosterBoard`                            | Direct-URL-only, header-free conference roster overview with a center switch between the complete active-and-inactive owner ladder and an enlarged live clock/recent/upcoming pick view with directional connectors. Each roster uses a readable, lightened palette derived from its team logo. Compact roster typography leaves room for remaining round-overall pick labels, which fill down columns and disappear on selection. Abbreviated names and fitted text keep each team in its panel.                                                                          |
| `/draft-roster-board/available` | Public, `noindex`        | `DraftAvailableTvBoard`                       | Direct-URL-only, header-free companion TV with full-height skater and goalie tables ordered by the same weighted composite `Rank` as the main draft Best Available table. That rank combines Yahoo, Daily Faceoff, and NHL draft rankings with the GSHL player-rating rank. The six highest-ranked available players are highlighted across both tables. The standalone rating, skater plus-minus, and penalty-minute columns are omitted. Both tables fit as many complete rows as the display height supports and contain no navigation to the app or other TV displays. |
| `/draft-roster-board/live`      | Public, `noindex`        | `DraftLiveTvBoard`                            | Direct-URL-only read-only draft flow: active team and clock across the top, height-fitted recent picks descending on the left, current-owner roster in the center, and height-fitted upcoming picks ascending on the right. Small color-coded chevrons connect each pick in its direction of travel. It contains no navigation to the app or other TV displays.                                                                                                                                                                                                            |

`src/app/not-found.tsx` supplies the global App Router not-found page by rendering `NotFoundContent` from `src/components/ui/NotFoundContent.tsx`. Dynamic matchup and edition screens currently render their own missing-data states instead of calling `notFound()`.
...
- Normal routes use `AppShell` and one `MainNavbar`. The shared season selector appears on Schedule and Standings in the mobile header and desktop top bar, so it does not consume a second navigation row. Historical season context is highlighted; desktop also includes a compact return to the current season. Mobile keeps labeled Home, Schedule, Standings, My Team, and More destinations at the bottom.
- More exposes Press Box, Rulebook, League Office, and Draft Hub. Commissioners also see a separate Admin destination for privileged tools and TV-display links. Matchup and detail routes keep their parent destination active; Matchup back links restore their allowlisted Schedule, Locker Room, or Press Box source context.
...
- Draft player pools use compact horizontal tables at every width, with abbreviated sticky player names on phones and a small sticky Draft button in every row. Selection confirmation stays visible in a modal; pending submissions and eligibility checks prevent duplicate or unauthorized actions. Both draft team views place roster, picks, and salary cap in that order on phones; desktop puts picks first, then roster and cap side by side. UFA decisions and matchup category/player statistics retain horizontally scrollable tables at every breakpoint, using truncated identity columns to preserve context on phones. NHL marks remain small beside player names, while GSHL team marks carry the stronger visual emphasis.
- Salary Cap remains a cross-season table at all widths with cap totals in its final row and nonsticky phone identity columns. Its window begins with the current operational contract season, or the upcoming season once the previous season has ended, independent of historical browsing context. Its Roster Planner exposes cap deltas and move cards at every width, with the complete scenario table in a disclosure. Franchise contract history uses cards below `lg` and its comprehensive table above that breakpoint.
...
- `/draft-roster-board` and its companion TV routes are public, bypass the shell entirely, and remain absent from member-facing navigation. Their only in-app links are inside the commissioner-only Admin TV Displays tab; the displays themselves contain no links to the main app or one another. Live TV uses the existing public season, team, player, and draft-pick reads with the shared draft-clock resolver; submitting or undoing picks remains restricted to the authenticated Draft Hub. Its roster follows the pick's franchise when the overview selects a newer season's team record.
- Schedule, Standings, Locker Room, League Office, and Draft layouts place one sticky context-navigation surface before route content. Schedule and Standings share the header season selector. My Team Matchups has a local multi-year filter; Draft Picks queries one franchise season at a time through its own selector. Neither changes the shared season context. These controls do not add fixed bottom bars or route-specific navigation padding.
- Navigation context is persisted in `gshl-nav-state` and mirrored to route-specific `view`, `season`, `week`, and `owner` query parameters. The persisted season is shared only by Schedule and Standings; other pages use current-season context or local year selectors. Valid URL state wins over hydrated persistence; user changes support Back/Forward, while automatic repair replaces the current entry.
...
/admin?view=tv
  → AdminContent
  → TvDisplays
  → public read-only draft TV routes
