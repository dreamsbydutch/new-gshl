...
var PlayerDataUpdater = (function () {
  "use strict";
   ...
   *
   * Usage: PlayerDataUpdater.updateAllPlayerData()
   */
  function updateAllPlayerData() {
    Logger.log("═══════════════════════════════════════════════════════════");
    Logger.log("👤 PLAYER DATA UPDATER");
    Logger.log("═══════════════════════════════════════════════════════════");
    ...
    try {
      ...
      var sheet = GshlUtils.sheets.open.getSheetByName(
        SPREADSHEET_ID,
        "Player",
        true,
      );
      ...
      try {
        ...
        if (season) {
          seasonTeams = GshlUtils.sheets.read
            .fetchSheetAsObjects(SPREADSHEET_ID, "Team")
            .filter((t) => String(t.seasonId) === String(season.id));
      ...
      for (var i = 1; i < data.length; i++) {
        ...
        if (shouldUpdateAges) {
          ...
          if (birthday) {
            var age = GshlUtils.domain.players.calculateAgeWithDecimal(
              birthday,
              currentDate,
            );
            if (age !== null) {
      ...
      if (nhlPosUpdates.length > 0) {
        ...
        Logger.log(
          "✅ Updated nhlPos for " + nhlPosUpdates.length + " players",
        );
      }
      ...
      if (lineupPosClearUpdates.length > 0) {
        ...
        Logger.log(
          "✅ Cleared lineupPos for " +
            lineupPosClearUpdates.length +
            " players",
        );
      }
      ...
      if (!season) {
        ...
        seasonTeams.forEach((team) => {
          ...
          var assignments = LineupBuilder.findBestLineup(roster, false);
          roster.forEach((p) => {
      ...
      Logger.log(
        "📅 Update date: " + GshlUtils.core.date.formatDate(currentDate),
      );
      Logger.log("═".repeat(60));
   ...
   *
   * Usage: PlayerAgeUpdater.testAgeCalculation()
   */
    ...
    try {
      ...
      for (var i = 1; i < data.length; i++) {
        if (String(data[i][idCol]) === String(playerId)) {
          ...
          var age = GshlUtils.domain.players.calculateAgeWithDecimal(birthday);
  ...
  function getMostRecentPlayerDayStats() {
    ...
    try {
      ...
      Logger.log(
        "   ❌ Error fetching PlayerDayStatLine data: " + error.message,
      );
      return { playerMap: {}, mostRecentDayKey: null };
    }
  }
  ...
  function debugPlayerData(playerId) {
    ...
    try {
      ...
      var playerSheet = GshlUtils.sheets.open.getSheetByName(
        SPREADSHEET_ID,
        "Player",
        true,
      );
      ...
      var statSheet = GshlUtils.sheets.open.getSheetByName(
        CURRENT_PLAYERDAY_SPREADSHEET_ID,
        "PlayerDayStatLine",
        false,
      );
      ...
      if (statSheet) {
        ...
        if (playerRecords.length > 0) {
          ...
          for (var k = 0; k < Math.min(5, playerRecords.length); k++) {
            ...
            Logger.log(
              "      " +
                (k + 1) +
                ". Date: " +
                GshlUtils.core.date.formatDate(rec.date) +
                " | nhlTeam: '" +
                rec.nhlTeam +
                "' | nhlPos: '" +
                rec.nhlPos +
          ...
          Logger.log(
            "      Date: " + GshlUtils.core.date.formatDate(mostRecent.date),
          );
  ...
  return {
    updateAllPlayerData: updateAllPlayerData,
    testAgeCalculation: testAgeCalculation,
    updatePlayerAgeById: updatePlayerAgeById,
    getMostRecentPlayerDayStats: getMostRecentPlayerDayStats,
    debugPlayerData: debugPlayerData,
  };
})();
