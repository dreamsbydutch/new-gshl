...
function getRelevantStats(posGroup, seasonId) {
  ...
  const usesPM = seasonId && parseInt(seasonId) >= 1 && parseInt(seasonId) <= 6;
...
function getAggregationBlendWeights(aggregationLevel, posGroup) {
  const trainedConfig =
    RANKING_MODELS &&
    RANKING_MODELS.aggregationBlendWeights &&
    RANKING_MODELS.aggregationBlendWeights[aggregationLevel];
