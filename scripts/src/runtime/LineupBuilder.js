...
var LineupBuilder = (function () {
  ...
  function dateOnlyFallback(d) {
    ...
    try {
      if (
        typeof LeagueRuntime !== "undefined" &&
        LeagueRuntime &&
        LeagueRuntime.core &&
        LeagueRuntime.core.date &&
        typeof LeagueRuntime.core.date.formatDateOnly === "function"
      ) {
        return LeagueRuntime.core.date.formatDateOnly(d);
      }
    } catch (_e) {
  ...
  function previousDateKeyFallback(dateKey, dateOnlyFn) {
    ...
    try {
      if (
        ...
        LeagueRuntime.core &&
        LeagueRuntime.core.date &&
        typeof LeagueRuntime.core.date.getPreviousDate === "function"
      ) {
        return String(LeagueRuntime.core.date.getPreviousDate(dateKey));
      }
    } catch (_e) {
      // ignore and use local fallback
    }
  ...
  function runExample() {
    ...
    if (
      ...
      console.log("=== Lineup Optimization Results ===");
      console.log("Full Lineup Rating: " + stats.fullPosRating.toFixed(2));
      console.log("Best Lineup Rating: " + stats.bestPosRating.toFixed(2));
      console.log(
        "Improvement: " +
