...
/**
 * PowerRankingsAlgo (local runtime)
 * ------------------------------
...
var PowerRankingsAlgo = (function buildPowerRankingsAlgo() {
  ...
  if (typeof LeagueRuntime === "undefined") {
    throw new Error(
      "LeagueRuntime is not defined. Ensure core utils are loaded before PowerRankingsAlgo.js",
    );
  }
  var readModel = LeagueRuntime.readModel;
  var formatDateOnly = LeagueRuntime.core.date.formatDateOnly;
  var toNumber = LeagueRuntime.core.parse.toNumber;
  var parseScore = LeagueRuntime.core.parse.parseScore;
  var normalizeSeasonId = LeagueRuntime.core.parse.normalizeSeasonId;

  var MATCHUP_CATEGORY_RULES =
    LeagueRuntime.core.constants.MATCHUP_CATEGORY_RULES;
  var SeasonType = LeagueRuntime.core.constants.SeasonType;
  ...
  function tryReadModel(dataModelName) {
    try {
      return readModel(dataModelName);
    } catch (e) {
      return null;
    }
  }
  ...
  function findPreviousSeasonId(seasonKey) {
    // Best-effort: use Season model ordering when present, else numeric decrement.
    var seasons = [];
    try {
      seasons = readModel("Season");
    } catch (e) {
      seasons = [];
    }
  ...
  function updatePowerRankingsForSeason(seasonId, options) {
    ...
    var seasons = tryReadModel("Season") || [];
    var allTeams = tryReadModel("Team") || [];
    var allFranchises = tryReadModel("Franchise") || [];
    var allTeamWeekRows = tryReadModel("TeamWeekStatLine") || [];
    var allTeamAwards =
      tryReadModel("TeamAwards") || tryReadModel("TeamAward") || [];
    var allPlayerWeekRows = tryReadModel("PlayerWeekStatLine") || [];
    var allMatchups = tryReadModel("Matchup") || [];

    var allWeeks = readModel("Week");
    ...
    var playerDayRows = tryReadModel("PlayerDayStatLine") || [];
    playerDayRows = playerDayRows.filter(function (row) {
      if (!row) return false;
      return String(row.seasonId || "") === String(seasonKey);
    });

    var playerNhlRows = tryReadModel("PlayerNHLStatLine") || [];
    ...
    var teamWeekRows = readModel("TeamWeekStatLine").filter(function (tw) {
      if (!tw) return false;
