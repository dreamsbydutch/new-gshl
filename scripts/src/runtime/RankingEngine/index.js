...
(function (ns) {
  ...
  var SKATER_DEFAULT_CATEGORIES = (
    TUNING_DEFAULTS.skaterCategories || [
      "G",
      "A",
      "P",
      "PPP",
  ...
  var LOWER_BETTER = {};
  (TUNING_DEFAULTS.lowerBetterStats || ["GAA", "GA"]).forEach(
    function (category) {
  ...
  function normalizeModelName(dataModelName) {
    var raw = String(dataModelName || "").trim();
    if (
      raw === "PlayerNHL" ||
      raw === "PlayerNhl" ||
  ...
  function detectModelName(row, options) {
    if (options && options.dataModelName)
      return normalizeModelName(options.dataModelName);
    if (!row) return "";
    ...
    if (row.gshlTeamIds !== undefined) {
      return row.seasonId ? "PlayerTotalStatLine" : "PlayerCareerTotalStatLine";
    }
    if (row.playerId && row.gshlTeamId) {
      return row.seasonId ? "PlayerSplitStatLine" : "PlayerCareerSplitStatLine";
    }
  ...
  function readSeasonCategories(seasonId) {
    ...
    try {
      var readModel = LeagueRuntime.readModel;
      if (!readModel) {
        seasonCategoryCache[seasonKey] = SKATER_DEFAULT_CATEGORIES.slice();
        return seasonCategoryCache[seasonKey].slice();
      }

      var seasons = readModel("Season") || [];
  ...
  function getRatingSkaterCategories(seasonId, dataModelName) {
    if (normalizeModelName(dataModelName) === "PlayerNHL") {
  ...
  function isPlayerAggregateModel(dataModelName) {
    var normalizedModelName = normalizeModelName(dataModelName);
    return (
      normalizedModelName === "PlayerDayStatLine" ||
      normalizedModelName === "PlayerWeekStatLine" ||
      normalizedModelName === "PlayerSplitStatLine" ||
      normalizedModelName === "PlayerTotalStatLine" ||
      normalizedModelName === "PlayerCareerSplitStatLine" ||
      normalizedModelName === "PlayerCareerTotalStatLine" ||
      normalizedModelName === "PlayerNHL"
    );
  }

  function isFullSeasonPlayerModel(dataModelName) {
    var normalizedModelName = normalizeModelName(dataModelName);
    return (
      normalizedModelName === "PlayerSplitStatLine" ||
      normalizedModelName === "PlayerTotalStatLine" ||
  ...
  function isSeasonTypeScopedPlayerAggregateModel(dataModelName) {
    var normalizedModelName = normalizeModelName(dataModelName);
    return (
      normalizedModelName === "PlayerSplitStatLine" ||
      normalizedModelName === "PlayerTotalStatLine" ||
  ...
  function getAggregateCategoryScoreMode(dataModelName) {
    var normalizedModelName = normalizeModelName(dataModelName);
    if (
      normalizedModelName === "PlayerDayStatLine" ||
      normalizedModelName === "PlayerWeekStatLine"
  ...
  function getOutputField(dataModelName, options) {
    if (options && options.outputField) return options.outputField;
    return normalizeModelName(dataModelName) === "PlayerNHL"
      ? "seasonRating"
      : "Rating";
  }
  ...
  function hasMeaningfulPlayerVolume(row, posGroup, dataModelName) {
    if (!row) return false;
  ...
  function getRetainedRangeShare(dataModelName, posGroup) {
    var normalizedModelName = normalizeModelName(dataModelName);
  ...
  function getDayWeekBaselineRows(rows, dataModelName, posGroup) {
    var normalizedModelName = normalizeModelName(dataModelName);
    if (
      normalizedModelName !== "PlayerDayStatLine" &&
      normalizedModelName !== "PlayerWeekStatLine"
    ...
    var playedRows = (rows || []).filter(function (row) {
      if (!hasMeaningfulPlayerVolume(row, posGroup, dataModelName))
        return false;
      if (normalizedModelName === "PlayerDayStatLine") {
        return getDailyPlayedFlag(row);
      }
      return getUsageValue(row, posGroup, dataModelName) > 0;
    });
  ...
  function getAggregateDistributionLimit(dataModelName, posGroup, seasonType) {
    var normalizedModelName = normalizeModelName(dataModelName);
    var modelLimits = PLAYER_AGGREGATE_DISTRIBUTION_LIMITS[normalizedModelName];
    if (!modelLimits) return 0;
    if (
      isSeasonTypeScopedPlayerAggregateModel(normalizedModelName) &&
      modelLimits[seasonType || "RS"]
    ) {
      return modelLimits[seasonType || "RS"][posGroup] || 0;
    }
    return modelLimits[posGroup] || 0;
  }
  ...
  function getScoreScale(dataModelName) {
    return 100;
  }

  function finalizeAggregateScore(score, dataModelName) {
    var numeric = Number(score);
    if (!isFinite(numeric)) return 0;
    return roundScore(Math.max(numeric, 0));
  }

  function applyAggregateModelCalibration(score, dataModelName, posGroup) {
    var normalizedModelName = normalizeModelName(dataModelName);
    ...
    if (normalizedModelName === "PlayerNHL") {
  ...
  function getUsageValue(row, posGroup, dataModelName) {
    if (posGroup === PositionGroup.G) {
  ...
  function getUsageRateValue(row, field, posGroup, dataModelName) {
    var usage = getUsageValue(row, posGroup, dataModelName);
    if (usage <= 0) return 0;
    return toNumber(row && row[field]) / usage;
  }

  function isWeeklyAggregateModel(dataModelName) {
    return normalizeModelName(dataModelName) === "PlayerWeekStatLine";
  }

  function shouldClampNonNegativeCategoryScore(dataModelName) {
    var normalizedModelName = normalizeModelName(dataModelName);
    return (
      normalizedModelName === "PlayerDayStatLine" ||
      normalizedModelName === "PlayerWeekStatLine"
  ...
  function clampCategoryScore(score, dataModelName) {
    var numericScore = Number(score);
    if (!isFinite(numericScore)) return 0;
    return shouldClampNonNegativeCategoryScore(dataModelName)
  ...
  function getComparableWeekUsageValue(usage, posGroup, dataModelName) {
    var numericUsage = Number(usage);
    if (!isFinite(numericUsage) || numericUsage <= 0) return 0;
    if (!isWeeklyAggregateModel(dataModelName)) return numericUsage;
  ...
  function getComparableUsageValue(row, posGroup, dataModelName) {
    return getComparableWeekUsageValue(
      getUsageValue(row, posGroup, dataModelName),
      posGroup,
      dataModelName,
    );
  }

  function scaleWeeklyCountingValue(value, row, posGroup, dataModelName) {
    ...
    var comparableUsage = getComparableUsageValue(row, posGroup, dataModelName);
    if (comparableUsage <= 0) return numericValue;

    return numericValue * (comparableUsage / usage);
  }

  function getComparableCategoryValue(row, category, posGroup, dataModelName) {
    var value = toNumber(row && row[category]);
    if (!isWeeklyAggregateModel(dataModelName)) return value;
    if (category === "GAA" || category === "SVP") return value;
    return scaleWeeklyCountingValue(value, row, posGroup, dataModelName);
  }
  ...
  function getComparableSkaterEventLoadValue(row, dataModelName) {
    var eventLoad = getSkaterEventLoadValue(row);
    if (!isWeeklyAggregateModel(dataModelName)) return eventLoad;
    return scaleWeeklyCountingValue(
      eventLoad,
      row,
      PositionGroup.F,
      dataModelName,
  ...
  function applySkaterTalentCategoryEmphasis(
    ...
    if (
      isFullSeasonPlayerModel(normalizedModelName) &&
      (posGroup === PositionGroup.F || posGroup === PositionGroup.D)
    ) {
      maxScore = 1.08;
      if (category === "P") {
        maxScore = normalizedModelName === "PlayerNHL" ? 1.14 : 1.1;
        if (adjusted > 0.72) adjusted += (adjusted - 0.72) * 0.22;
        if (adjusted > 0.88) {
          adjusted +=
            (adjusted - 0.88) *
            (normalizedModelName === "PlayerNHL" ? 0.28 : 0.18);
        }
      }
    }
  ...
  function getSkaterCategoryWeights(dataModelName, posGroup, categories) {
    var normalizedModelName = normalizeModelName(dataModelName);
    var weights = {};
  ...
  function getSkaterProfile(dataModelName) {
    return (
      SKATER_LEVEL_PROFILES[normalizeModelName(dataModelName)] ||
      SKATER_LEVEL_PROFILES.PlayerTotalStatLine
    );
  }

  function getGoalieProfile(dataModelName) {
    return (
      GOALIE_LEVEL_PROFILES[normalizeModelName(dataModelName)] ||
  ...
  ns.EngineDeps = {
    ...
    applyAggregateModelCalibration: applyAggregateModelCalibration,
    finalizeAggregateScore: finalizeAggregateScore,
    getGoalieToiValue: getGoalieToiValue,
  };

  function buildPrimaryGroupKey(row, dataModelName, posGroup) {
    var seasonId =
      row && row.seasonId !== undefined && row.seasonId !== null
        ? String(row.seasonId)
        : "";
    var seasonType = getAggregateSeasonTypeKey(row);
    if (isPlayerAggregateModel(dataModelName)) {
      if (isSeasonTypeScopedPlayerAggregateModel(dataModelName)) {
        return [seasonId, dataModelName, seasonType, posGroup].join("|");
      }
      return [seasonId, dataModelName, posGroup].join("|");
    }
    return [seasonId, posGroup].join("|");
  }

  function buildFallbackGroupKey(row, dataModelName, posGroup) {
    var seasonId =
      row && row.seasonId !== undefined && row.seasonId !== null
        ? String(row.seasonId)
        : "";
    ...
    return buildPrimaryGroupKey(row, dataModelName, posGroup);
  }
  ...
  function buildAggregateGroupContext(rows, dataModelName) {
    return {
  ...
  function getPoolRowsForGroup(groupRows, dataModelName, context, posGroup) {
    var threshold =
    ...
    var validPrimaryRows = (groupRows || []).filter(function (row) {
      return hasMeaningfulPlayerVolume(row, posGroup, dataModelName);
    });
    if (validPrimaryRows.length >= threshold) return validPrimaryRows;

    var fallbackKey = buildFallbackGroupKey(
      groupRows[0],
      dataModelName,
      posGroup,
    );
  ...
  function fetchSeasonTeamDayRows(seasonId) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(teamDaySeasonRowsCache, seasonKey)
    ) {
  ...
  function computeTeamSeasonAwardRatings(
    rows,
    categories,
    ratingField,
    rankField,
  ...
  function isTruthyModelValue(value) {
  ...
  function mapPlayerSplitsByTeamPlayer(rows) {
    ...
    (rows || []).forEach(function (row) {
      ...
      if (
        !teamKey ||
        !playerKey ||
        !isRegularSeasonType(row && row.seasonType)
      ) {
        return;
      }
  ...
  function buildLeagueTalentFallback(
    previousNhlRows,
    currentNhlRows,
    totalRows,
  ) {
  ...
  function buildTeamAwardComponentContext(regularRows, seasonId) {
    ...
    var improvementValues = Object.keys(teamTalentValues).map(
      function (teamKey) {
    ...
    (regularRows || []).forEach(function (row) {
      ...
      var winsScore = percentileScore(
        toNumber(row && row.teamW),
        winsValues,
        false,
      );
    ...
    var overPerformanceValues = Object.keys(componentsByTeam).map(
      function (teamKey) {
  ...
  function buildCalderTeamScores(regularRows, seasonId, componentContext) {
    ...
    Object.keys(picksByTeam).forEach(function (teamKey) {
      ...
      result[teamKey] = {
        score: roundScore(
          latePicks.length ? topScore * 0.8 + lateScore * 0.2 : topScore,
        ),
        hasPicks: true,
      };
    });
  ...
  function rankTeamSeasonAwardRows(
    rows,
    ratingField,
    rankField,
    eligibilityMap,
  ...
  function applyTeamSeasonAwardRatings(rows, seasonId) {
    ...
    var norrisByTeamKey = {};
    var vezinaByTeamKey = {};
    var componentContext = buildTeamAwardComponentContext(
      regularRows,
      seasonId,
    );
    ...
    rankTeamSeasonAwardRows(regularRows, "jackAdamsRating", "jackAdamsRk");
  ...
  function rankRows(rows, options) {
    var opts = options || {};
    var dataModelName = normalizeModelName(
      opts.dataModelName ||
        (rows && rows[0] ? detectModelName(rows[0], opts) : ""),
    );
    var outputField = getOutputField(dataModelName, opts);
    var targetRows =
      opts.mutate === false ? (rows || []).map(cloneObject) : rows || [];

    if (!targetRows.length) return targetRows;

    if (dataModelName === "TeamDayStatLine") {
      var seasonGroups = {};
      targetRows.forEach(function (row) {
        var seasonKey =
          row && row.seasonId !== undefined ? String(row.seasonId) : "";
    ...
    if (dataModelName === "TeamWeekStatLine") {
    ...
    if (dataModelName === "TeamSeasonStatLine") {
      ...
      Object.keys(teamSeasonGroups).forEach(function (groupKey) {
        rankSeasonScopedTeamSeasonRows(teamSeasonGroups[groupKey], outputField);
      });
      return targetRows;
    }

    if (dataModelName.indexOf("Team") === 0) {
      rankTeamRows(targetRows, outputField);
      return targetRows;
    }

    var context = buildAggregateGroupContext(targetRows, dataModelName);
    Object.keys(context.primaryGroups).forEach(function (key) {
      ...
      var poolRows = getPoolRowsForGroup(
        group,
        dataModelName,
        context,
        posGroup,
      ...
      if (posGroup === PositionGroup.G) {
        rankGoalieGroup(group, poolRows, dataModelName, outputField);
      } else if (posGroup === PositionGroup.TEAM) {
        rankTeamRows(group, outputField);
      } else {
        rankSkaterGroup(group, poolRows, dataModelName, outputField, opts);
      }
    });
  ...
  function rankTeamSingle(row, dataModelName) {
    var normalizedModelName = normalizeModelName(dataModelName);

    if (normalizedModelName === "TeamDayStatLine") {
      var categories = getMatchupCategoriesForSeason(row && row.seasonId);
      if (!hasMeaningfulTeamDayActivity(row, categories)) {
        return buildResult(row, dataModelName, PositionGroup.TEAM, 0, [], {
          categoryQuality: 0,
          spike: 0,
          breadth: 0,
          volume: 0,
      ...
      var resolvedRow = hasExistingRow
        ...
        : targetRow;
      var score =
        resolvedRow && resolvedRow.Rating !== undefined
          ? resolvedRow.Rating
          : 0;
      ...
      return buildResult(
        ...
        [],
        {
          categoryQuality: normalizedScore / getScoreScale(dataModelName),
          spike: normalizedScore / getScoreScale(dataModelName),
    ...
    if (normalizedModelName === "TeamWeekStatLine") {
      ...
      rankSeasonScopedTeamWeekRows(poolRows, getOutputField(dataModelName, {}));
    ...
    if (normalizedModelName === "TeamSeasonStatLine") {
      ...
      var hasExistingTeamSeasonRow = teamSeasonPoolRows.some(
        function (poolRow) {
      ...
      return buildResult(
        ...
        [],
        {
          categoryQuality: normalizedSeasonScore / getScoreScale(dataModelName),
          spike: normalizedSeasonScore / getScoreScale(dataModelName),
  ...
  function rankAggregateSingle(row, dataModelName, posGroup) {
    var outputField = getOutputField(dataModelName, {});
  ...
  function blankResult(row, dataModelName, posGroup) {
    return buildResult(row, dataModelName, posGroup, "", [], {
      categoryQuality: 0,
      spike: 0,
      breadth: 0,
      volume: 0,
  ...
  function buildResult(
    ...
    return {
      ...
      aggregationLevel: dataModelName,
      posGroup: posGroup,
      ...
      rawComposite:
        score === ""
          ? 0
          : Number(score) /
            (normalizeModelName(dataModelName) === "TeamWeekStatLine"
              ? getScoreScale(dataModelName)
              : 125),
    };
  }
  ...
  function rankPerformance(row, options) {
    var opts = options || {};
    var dataModelName = normalizeModelName(detectModelName(row, opts));
    var posGroup = normalizePosGroup(row && row.posGroup, row);

    if (
      row &&
      row.playerId &&
      (dataModelName === "PlayerDayStatLine" ||
        dataModelName === "PlayerWeekStatLine")
    ) {
      return blankResult(row, dataModelName, posGroup);
    }
    if (
      posGroup === PositionGroup.TEAM ||
      dataModelName.indexOf("Team") === 0
    ) {
      return rankTeamSingle(row, dataModelName);
    }
    return rankAggregateSingle(row, dataModelName, posGroup);
  }
