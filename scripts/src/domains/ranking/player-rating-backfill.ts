...
export async function executePlayerRatingModelBackfill(
  ...
  if (modelName === "PlayerDayStatLine") {
    applyPlayerDayDerivedColumns(
      ...
      prepared.rows
        ...
        .filter(
          (record) => toTrimmedString(record.seasonId) === options.seasonId,
        ),
    );
  }
  ...
  for (const row of targetRecords) {
    const normalizedRating =
      row[prepared.outputField] === "" ||
      row[prepared.outputField] === null ||
      row[prepared.outputField] === undefined
        ? 0
        : row[prepared.outputField];
    row[prepared.outputField] = normalizedRating;
    if (prepared.outputField !== "Rating") {
      row.Rating = normalizedRating;
    }
  }
