 ...
 *   npm run player-bios:sync -- --headless --page-size 100 --max-pages 25
 *
 * What it does:
 *   Scrapes player bio and roster context from PuckPedia, matches that data to
 *   the Player sheet, and prepares updates or inserts for bios, rankings,
 *   roster context, and lineup-derived fields. By default it opens a browser,
 *   may wait for manual Cloudflare clearance, and runs as a dry-run unless
 *   --apply is passed.
 *
 * Options:
...
import type { OptimizedSheetsClient } from "../../integrations/sheets/client/optimized-client";
import {
  getPlayerDayWorkbookId,
  PLAYERDAY_WORKBOOK_KEYS,
  SHEETS_CONFIG,
  WORKBOOKS,
} from "../../integrations/sheets/config/config";
...
async function getOptimizedSheetsClient(): Promise<OptimizedSheetsClient> {
  ...
  optimizedSheetsClientPromise ??= import(
    "../../integrations/sheets/client/optimized-client"
