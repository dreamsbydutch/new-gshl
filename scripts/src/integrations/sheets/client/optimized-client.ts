...
export class OptimizedSheetsClient {
  ...
  async deleteRows(
    ...
    if (!uniqueDescendingRowNumbers.length) {
      return;
    }

    const metadata = await this.getSheetMetadata(spreadsheetId);
    const sheet = metadata.find((entry) => entry.title === sheetName);
    if (!sheet) {
      throw new Error(`Sheet metadata not found for ${sheetName}.`);
    }

    const requests = uniqueDescendingRowNumbers.map((rowNumber) => ({
      deleteDimension: {
        range: {
  ...
  private shouldRetry(
    ...
    const isQuotaError =
      ...
      (errorMessage.includes("quota exceeded") ||
        ...
        errorMessage.includes("write requests per minute per user"));
    return (
      statusCode === 429 ||
      isQuotaError ||
  ...
  async updateRowsByIds(
    ...
    for (const [rowId, values] of sortedUpdates) {
      ...
      if (!canExtendGroup) {
        if (currentGroup) {
          data.push({
            range: `${sheetName}!A${currentGroup.startRowId + 1}:${this.getColumnLetter(currentGroup.columnCount)}${currentGroup.endRowId + 1}`,
