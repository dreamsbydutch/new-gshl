...
function extractRawBuckets(img: HTMLImageElement): Bucket[] | null {
  try {
    ...
    for (let p = 0; p < totalPixels; p += stride) {
      ...
      const g = data[i + 1] ?? 0;
      const b = data[i + 2] ?? 0;
      // Skip near-white/near-black low-information background early; we'll still fallback if nothing else
...
function choosePrimary(palette: string[], buckets: Bucket[]): string | null {
  ...
  // favor first palette color that isn't too light/dark and appears in a bucket with decent saturation
  const byHex = new Map(
    buckets.map(
      (b) =>
        [
  ...
  const candidate = palette.find((hex) => {
    const b = byHex.get(hex);
    return b && b.saturation > 0.15 && b.brightness > 25 && b.brightness < 230;
  });
...
export interface TeamPaletteResult extends TeamPaletteCacheEntry {}
...
export function useTeamPalette(logoUrl?: string | null): TeamPaletteResult {
  const [state, setState] = useState<TeamPaletteResult>(() =>
    logoUrl && paletteCache.has(logoUrl)
      ? (paletteCache.get(logoUrl) as TeamPaletteResult)
