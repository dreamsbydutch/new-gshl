...
import { getAwardLabel, getTeamAwardTeam } from "@gshl-lib/config/awards";
import type {
        ...
        <div className="mt-7 grid grid-cols-[1fr_auto_1fr] items-center gap-2 sm:gap-8">
          ...
          <div className="text-center">
            ...
            <div className="mt-1 flex items-center gap-1 font-oswald text-5xl font-bold tabular-nums drop-shadow-lg sm:gap-3 sm:text-8xl">
              ...
              <span className="text-2xl font-normal text-white/35 sm:text-4xl">
                :
              </span>
...
function ComponentCard({
  ...
  const leftValue =
    rating.componentsByConferenceId[left.id]?.[component.key] ?? 50;
  const leftDisplay = Math.round(leftValue);
  const rightDisplay = 100 - leftDisplay;
  const leftTone = conferenceTone(left);
  const rightTone = conferenceTone(right);
    ...
    <article className="rounded-3xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-4 shadow-[0_15px_35px_rgba(15,23,42,0.07)] sm:p-5">
      ...
      <div className="mt-2 flex h-3 overflow-hidden rounded-full bg-slate-100 shadow-inner">
        ...
        <div
          ...
          style={{ backgroundColor: rightTone.strong }}
          aria-hidden="true"
        />
      </div>
  ...
  return (
    <section className="rounded-[2rem] border border-slate-200 bg-white p-4 shadow-[0_18px_45px_rgba(15,23,42,0.08)] sm:p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-barlow text-[10px] uppercase tracking-[0.25em] text-slate-400">
            Season by season
          </p>
          ...
          <p className="mt-1 text-sm text-slate-500">
            How each conference&apos;s single-season rating has moved over time.
          </p>
        </div>
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {teams.map((team, index) => (
        <TeamMark key={`${team.id}-${index}`} team={team} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center gap-2 text-slate-500">
        ...
        <h4 className="font-barlow text-xs uppercase tracking-[0.18em]">
          {title}
        </h4>
      </div>
      <div className="mt-4 grid grid-cols-[1fr_auto_1fr] items-start gap-3 text-center">
        <div>
          <p className={cn("font-oswald text-3xl", conferenceTone(left).text)}>
            {leftAwards.length}
          </p>
          <p className="mt-1 truncate text-xs text-slate-500">
            {latestWinner(leftAwards)}
          </p>
        </div>
        <div className="pt-2 text-xs text-slate-300">VS</div>
        <div>
          <p className={cn("font-oswald text-3xl", conferenceTone(right).text)}>
            {rightAwards.length}
          </p>
          <p className="mt-1 truncate text-xs text-slate-500">
            {latestWinner(rightAwards)}
          </p>
        </div>
      </div>
    </div>
...
function StatBlock({
  ...
  return (
    <div className="grid grid-cols-[1fr_7rem_1fr] items-center gap-2 border-b border-slate-100 py-3 last:border-0 sm:grid-cols-[1fr_10rem_1fr]">
      <div className="text-right font-oswald text-lg text-slate-900">
        {left}
      </div>
      <div className="text-center font-barlow text-[10px] uppercase tracking-[0.15em] text-slate-400">
        {label}
      </div>
...
function SeasonExplorer({
  ...
  const leftRating = Math.round(
    selectedSeason.ratingByConferenceId[leftId] ?? 50,
  );
      ...
      <div className="border-b border-slate-200 bg-white p-4 sm:p-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          ...
          <label className="relative block">
            ...
            <ChevronDown
              className="pointer-events-none absolute right-3 top-3.5 h-4 w-4 text-slate-400"
              aria-hidden="true"
            />
          </label>
        </div>
      ...
      <div className="p-4 sm:p-6">
        <div className="rounded-3xl bg-slate-950 px-4 py-5 text-white sm:px-6">
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
            <div className="text-center">
              <p className="font-barlow text-[10px] uppercase tracking-[0.18em] text-white/55">
                {cleanConferenceName(left.name)}
              </p>
              <p className="mt-1 font-oswald text-5xl tabular-nums">
                {leftRating}
              </p>
            </div>
            <div className="text-center">
              <p className="font-barlow text-[9px] uppercase tracking-[0.2em] text-white/40">
                {selectedSeason.seasonYear}
              </p>
              <Swords
                className="mx-auto mt-1 h-5 w-5 text-white/30"
                aria-hidden="true"
              />
            </div>
            <div className="text-center">
        ...
        <div className="mt-5 rounded-2xl border border-slate-200 bg-white px-3 sm:px-5">
          <StatBlock
            label="H2H record"
            left={recordLabel(
              selectedSeason.headToHeadRecordByConferenceId[leftId],
            )}
            right={recordLabel(
          ...
          <StatBlock
            ...
            left={recordLabel(
              selectedSeason.playoffRecordByConferenceId[leftId],
            )}
            right={recordLabel(
              selectedSeason.playoffRecordByConferenceId[rightId],
            )}
          />
          <StatBlock
            label="Finalists"
            ...
            right={
              selectedSeason.finalsTeamsByConferenceId[rightId]?.length ?? 0
            }
          />
          <StatBlock
            label="Award points"
            left={selectedSeason.awardPointsByConferenceId[leftId] ?? 0}
            right={selectedSeason.awardPointsByConferenceId[rightId] ?? 0}
          />
        </div>
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {[left, right].map((conference) => {
            ...
            const champions =
              selectedSeason.championTeamsByConferenceId[conferenceId] ?? [];
            return (
              <article
                key={conferenceId}
                className={cn(
                  "rounded-3xl border p-4 sm:p-5",
                  conferenceTone(conference).soft,
                )}
              >
                <div className="flex items-center gap-3">
                  <ConferenceLogo conference={conference} size={44} />
                  <div>
                    <p className="font-barlow text-[10px] uppercase tracking-[0.2em] text-slate-400">
                      {conference.abbr}
                    </p>
                    <h3 className="font-oswald text-xl text-slate-950">
                ...
                <div className="mt-5 space-y-5">
                  <div>
                    <h4 className="mb-2 flex items-center gap-2 font-barlow text-[10px] uppercase tracking-[0.18em] text-slate-500">
                      <Trophy className="h-4 w-4" /> Cup winner
                    </h4>
                    <TeamCollection
                      teams={champions}
                      empty={
                  ...
                  <div>
                    <h4 className="mb-2 flex items-center gap-2 font-barlow text-[10px] uppercase tracking-[0.18em] text-slate-500">
                      <Users className="h-4 w-4" /> Playoff teams
                    </h4>
                    <TeamCollection
                      teams={
                        selectedSeason.playoffTeamsByConferenceId[
                          conferenceId
                        ] ?? []
                      }
                    />
                  ...
                  <div>
                    <h4 className="mb-2 flex items-center gap-2 font-barlow text-[10px] uppercase tracking-[0.18em] text-slate-500">
                      <Crown className="h-4 w-4" /> Finals
                    </h4>
                    <TeamCollection
                      teams={
                        selectedSeason.finalsTeamsByConferenceId[
                          conferenceId
                        ] ?? []
                      }
                    />
        ...
        <div className="mt-5 rounded-3xl border border-slate-200 bg-white p-4 sm:p-5">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-oswald text-xl text-slate-950">
              {selectedSeason.seasonName} award winners
            </h3>
          </div>
          {allAwards.length ? (
            <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {allAwards.map((award) => {
                const team = getTeamAwardTeam(award, [...teamsById.values()]);
                return (
                  <div
                    key={award.id}
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="truncate font-oswald text-base text-slate-900">
                        {getAwardLabel(award.award)}
                      </p>
                      <p className="truncate text-xs text-slate-500">
                        {team?.ownerNickname ?? team?.name ?? "Winner"}
                      </p>
                    </div>
                  </div>
                );
            ...
            <p className="mt-3 text-sm text-slate-400">
              No eligible league awards are recorded for this season yet.
            </p>
          )}
        </div>
...
export function ConferenceContest() {
  const { overall, seasons, teams, isLoading, error } =
    useConferenceContestData();
    ...
    return (
      <div className="mx-auto max-w-2xl rounded-3xl border border-red-200 bg-red-50 p-8 text-center">
        ...
        <h1 className="mt-3 font-oswald text-2xl text-red-950">
          Conference Battle is unavailable
        </h1>
    ...
    return (
      <div className="mx-auto max-w-2xl rounded-3xl border border-slate-200 bg-slate-50 p-8 text-center">
        ...
        <h1 className="mt-3 font-oswald text-2xl text-slate-900">
          The battle is waiting to begin
        </h1>
        <p className="mt-2 text-sm text-slate-500">
          Two conferences and at least one season are needed to build the
          comparison.
        </p>
      </div>
    );
  ...
  const rating =
    mode === "current" ? overall.currentRating : overall.allTimeRating;
    ...
    <div className="mx-auto max-w-6xl space-y-6 pb-8">
      <header className="px-1 text-center sm:text-left">
        <p className="font-barlow text-xs uppercase tracking-[0.3em] text-slate-400">
          GSHL Conference Battle
        </p>
        <h1 className="mt-1 font-oswald text-4xl uppercase leading-none text-slate-950 sm:text-6xl">
          Who runs the league?
        </h1>
        <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-500 sm:mx-0 sm:text-base">
          Every matchup, playoff run, Cup and major award—distilled into one
          living head-to-head rating.
        </p>
      </header>
      ...
      <section className="rounded-[2rem] border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-slate-50 p-4 shadow-[0_18px_45px_rgba(15,23,42,0.07)] sm:p-6">
        <div className="flex items-start gap-3">
          <div className="rounded-2xl bg-amber-400 p-2.5 text-amber-950 shadow-lg">
            <Crown className="h-5 w-5" />
          </div>
          <div>
            <p className="font-barlow text-[10px] uppercase tracking-[0.24em] text-amber-700">
              Leadership edge
            </p>
            <h2 className="font-oswald text-2xl text-slate-950 sm:text-3xl">
              The awards that shape a conference
            </h2>
            <p className="mt-1 text-sm text-slate-500">
              Coach and GM of the Year each carry triple weight in the battle.
            </p>
          </div>
        </div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          <LeadershipCard
            ...
            icon={Crown}
            leftAwards={overall.coachAwardsByConferenceId[left.id] ?? []}
            rightAwards={overall.coachAwardsByConferenceId[right.id] ?? []}
            left={left}
            right={right}
            teamsById={teamsById}
          />
          <LeadershipCard
            ...
            icon={Shield}
            leftAwards={overall.gmAwardsByConferenceId[left.id] ?? []}
            rightAwards={overall.gmAwardsByConferenceId[right.id] ?? []}
            left={left}
            right={right}
            teamsById={teamsById}
          />
      ...
      <details className="group rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                A transparent, evidence-based comparison
              </p>
            </div>
          </div>
          <ChevronDown
        ...
        <div className="border-t border-slate-100 px-5 pb-6 pt-5 text-sm leading-6 text-slate-600 sm:px-6">
          <p>
            The rating always totals 100. Head-to-head results count for 35%,
            playoff performance for 30%, Cups for 20%, and league awards for
            15%. Playoff performance blends wins (50%), Finals appearances
            (30%), and qualifying teams (20%).
          </p>
          <p className="mt-3">
            Coach of the Year and GM of the Year are worth three award points;
            other eligible league-wide awards are worth one. Conference trophies
            and All-Star selections are excluded, and the Cup is counted only
            once in its own category.
          </p>
          <p className="mt-3">
            Current form retains 85% of the previous season&apos;s weight -
            1.00, 0.85, 0.72, 0.61 and so on - while All Time treats every
            season equally. Missing or undecided evidence stays neutral at
            50-50.
          </p>
        </div>
      </details>
