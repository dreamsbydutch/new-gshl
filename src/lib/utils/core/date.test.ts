...
void test("coerceDate treats ISO datetimes differently by mode", () => {
  const isoDateTime = "2026-09-07T14:30:00.000Z";

  assert.equal(
    coerceDate({ value: isoDateTime, mode: "date-only" })?.toISOString(),
    "2026-09-07T00:00:00.000Z",
  );
  assert.equal(
    coerceDate({ value: isoDateTime, mode: "instant" })?.toISOString(),
    isoDateTime,
  );
});
