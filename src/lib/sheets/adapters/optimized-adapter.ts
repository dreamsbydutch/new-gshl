...
export class OptimizedSheetsAdapter {
  ...
  private getSpreadsheetId(modelName: string, customId?: string): string {
    // Allow custom spreadsheet ID override (used by PlayerDayAdapter)
    if (customId) {
      return customId;
    }
  ...
  /**
   * Load data directly from a custom spreadsheet without caching
   * Used by PlayerDayAdapter for partitioned data
   */
  private async loadDirectFromSpreadsheet<T extends DatabaseRecord>(
    modelName: string,
    spreadsheetId: string,
  ): Promise<T[]> {
    const sheetName = SHEETS_CONFIG.SHEETS[modelName as ModelName];
    const columns = SHEETS_CONFIG.COLUMNS[modelName as ModelName];
    ...
    const range = `${sheetName}!A2:${this.getColumnLetter(columns.length)}`;
    const values = await this.sheetsApi().getValues(spreadsheetId, range);

    const results: T[] = [];
    values.forEach((row: (string | number | boolean | null)[]) => {
      if (!row || row.length === 0) return;
      const model = convertRowToModel<T>(row, columns);
      results.push(model);
    });
  ...
  async findMany<T extends DatabaseRecord>(
    ...
    return this.findManyWithSpreadsheet<T>(modelName, options);
  }

  /**
   * Find many records with optional custom spreadsheet ID
   * Used internally and by PlayerDayAdapter for partitioned queries
   */
  async findManyWithSpreadsheet<T extends DatabaseRecord>(
    modelName: ModelName,
    options: FindManyOptions<T> = {},
    ...
    try {
      ...
      if (customSpreadsheetId) {
        // Direct load without caching for custom spreadsheets
        results = await this.loadDirectFromSpreadsheet<T>(
          modelName,
          customSpreadsheetId,
        );
      } else {
        // Use cached dataset for standard queries
        const dataset = await this.getDataset<T>(modelName);
        results = dataset.rows.map((record: SheetRow<T>) => record.model);
      }
  ...
  /**
   * Bulk update multiple records by ID with a single range write.
   * Assumes records are in a contiguous block (e.g., all records for a single date).
   * Much faster than individual updates - writes entire range at once.
   */
  async bulkUpdateByIds<T extends DatabaseRecord>(
    modelName: ModelName,
    ...
    try {
      // Get the dataset to map IDs to row indices
      const dataset = await this.getDataset<DatabaseRecord>(modelName);
      const now = new Date();

      // Build map of ID to update data and collect row indices
      const updateMap = new Map<string, Partial<T>>();
      const rowIndices: number[] = [];

      for (const { id, data: updateData } of updates) {
        const rowIndex = dataset.idToRowIndex.get(String(id));
        if (!rowIndex) {
          console.warn(`Record with id ${id} not found in ${modelName}`);
          continue;
        }
        updateMap.set(String(id), updateData);
      ...
      // Sort row indices to find the contiguous range
      rowIndices.sort((a, b) => a - b);
      const minRow = rowIndices[0]!;
      const maxRow = rowIndices[rowIndices.length - 1]!;

      // Fetch the entire range of existing records in one API call
      const range = `${sheetName}!A${minRow}:${this.getColumnLetter(columns.length)}${maxRow}`;
      const existingRows = await this.sheetsApi().getValues(
        this.getSpreadsheetId(String(modelName)),
        range,
      );
      ...
      for (let i = 0; i < existingRows.length; i++) {
        ...
        const existingData = convertRowToModel<T>(existingRow, columns);
        const recordId = String((existingData as Record<string, unknown>).id);
        const updateData = updateMap.get(recordId);
        ...
        if (updateData) {
          ...
          // Preserve createdAt if not explicitly updated
          if (
            "createdAt" in existingData &&
            existingData.createdAt &&
            !("createdAt" in updateData)
          ) {
            (updatedData as T & WithTimestamps).createdAt = (
              existingData as T & WithTimestamps
            ).createdAt;
          ...
          updatedRows.push(convertModelToRow(updatedData, columns));
        } else {
          // Row is in the range but not being updated - keep as is
          updatedRows.push(existingRow as (string | number | boolean)[]);
        }
      }

      // Write entire range in ONE API call
      await this.sheetsApi().updateValues(
        this.getSpreadsheetId(String(modelName)),
        range,
        updatedRows,
      ...
      this.clearCacheForModel(String(modelName));
      return { count: updateMap.size };
    } catch (error) {
      console.error(`Error bulk updating ${modelName}:`, error);
      throw error;
    }
  }
